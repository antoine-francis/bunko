...
export const Login = () => {
	const {loading, error} = useBunkoSelector(state => state.currentUser);
	...
	const user : UserBadge | undefined = useSession();

	useEffect(() => {
		const redirection : string | null = params.get("redirectTo");
		if (user && !loading) {
			if (redirection) {
